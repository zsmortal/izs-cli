...
export const promptActions = [
  ...
  },
  {
    ...
    message: '项目名称：',
    default: (answers) => {
      return projectList[answers.index].projectName
    },
    validate: (val) => {
      if (!val) return '项目名称不能为空'
      return true
  ...
  },
  {
    ...
    message: '项目描述：',
    default: (answers) => {
      return projectList[answers.index].desc
    },
    validate: (val) => {
      if (!val) return '项目描述不能为空（或项目中文名称）'
      return true
  ...
  },
  {
    ...
    message: '本地打包文件目录：',
    default: (answers) => {
      return projectList[answers.index].localDir
    },
    validate: (val) => {
      if (!val) return '本地打包文件目录不能为空'
      return true
  ...
  },
  {
    ...
    message: '远程文件部署目录：',
    default: (answers) => {
      return projectList[answers.index].remoteDir
    },
    validate: (val) => {
      if (!val) return '远程文件部署目录不能为空'
      return true
  ...
  },
  {
    ...
    message: '文件的系统绝对路径：',
    validate: (val) => {
      if (val.indexOf('.txt') === -1) return '请上传一个.txt文件，格式：[{"projectName":"","desc":"","localDir":"","remoteDir":""}]'
      return true
    },
    when: (answers) => {
