...
const serverPath = getFilePath('../../file/server.txt')
    ...
    inquirer
      ...
      .then((answers) => {
        if (answers.mode === 'add') {
          try {
            delete answers.mode
            serverList.push(answers)
            fs.writeFileSync(serverPath, JSON.stringify(serverList))
          ...
          try {
            ...
            fs.writeFileSync(serverPath, JSON.stringify([...serverList, ...JSON.parse(data)]))
          ...
          if (answers.mode === 'delete') {
            try {
              serverList.splice(Number(answers.index), 1)
              fs.writeFileSync(serverPath, JSON.stringify(serverList))
            ...
            try {
              delete answers.mode
              serverList[Number(answers.index)] = answers
              fs.writeFileSync(serverPath, JSON.stringify(serverList))
            ...
            console.table(serverList, Object.keys(serverList[0]))
          } else if (answers.mode === 'download') {
            try {
              shell.cp(serverPath, getHomeDir())
