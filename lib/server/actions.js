...
export const promptActions = [
  ...
  },
  {
    ...
    message: '服务器IP地址：',
    default: (answers) => {
      return serverList[answers.index].host
    },
    validate: (val) => {
      if (!val) return '服务器IP地址不能为空'
      return true
  ...
  },
  {
    ...
    message: '服务器端口号：',
    default: (answers) => {
      return serverList[answers.index].port
    },
    validate: (val) => {
      if (!val) return '服务器端口号不能为空'
      return true
  ...
  },
  {
    ...
    message: '服务器账号：',
    default: (answers) => {
      return serverList[answers.index].userName
    },
    validate: (val) => {
      if (!val) return '服务器账号不能为空'
      return true
  ...
  },
  {
    ...
    message: '服务器密码：',
    default: (answers) => {
      return serverList[answers.index].passWord
    },
    validate: (val) => {
      if (!val) return '服务器密码不能为空'
      return true
  ...
  },
  {
    ...
    message: '服务器描述：',
    default: (answers) => {
      return serverList[answers.index].desc
    },
    validate: (val) => {
      if (!val) return '服务器描述不能为空（如线上/测试服务器）'
      return true
  ...
  },
  {
    ...
    message: '文件的系统绝对路径：',
    validate: (val) => {
      if (val.indexOf('.txt') === -1) return '请上传一个.txt文件，格式：[{"host":"","port":"","userName":"","passWord":"","desc":""}]'
      return true
    },
    when: (answers) => {
