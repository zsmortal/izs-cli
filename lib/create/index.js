...
import { getFileContent, isFileExisted } from '../../utils/tool.js'
      ...
      inquirer
        ...
        .then((answers) => {
          if (answers.projectCover) {
            ...
            try {
              shell.rm('-rf', `${process.cwd()}/${appName}`)
  ...
  try {
    inquirer
      ...
      .then((answers) => {
        ...
        gitclone(answers.templateUrl, `${process.cwd()}/${appName}`, { checkout: 'master', shallow: true }, (err) => {
          if (err) {
            initSpinner.fail(chalk.red(`模板下载失败：${new Error(err)}`)).stop()
            process.exit()
          } else {
            initSpinner.succeed(chalk.green('模版下载成功')).stop()
            shell.rm('-rf', `${process.cwd()}/${appName}/.git`)
            shell.cd(appName)
